...
#define DEBUG(__fmt, __args...) \
	do { \
		FILE *fp; \
		fp = fopen("/tmp/log", "a"); \
		fprintf(fp, __fmt, ##__args); \
		fclose(fp); \
	} while(0);
#define DEBUG_BLOC(__ptr, __len) \
	do { \
		FILE *fp; \
		fp = fopen("/tmp/log", "a"); \
		fprintf(fp, "%p %d\n", s->arg, a->c.content_len); \
		fprintf(fp, "==================================================================\n"); \
		fwrite(__ptr, 1, __len, fp); \
		fprintf(fp, "\n==================================================================\n"); \
		fclose(fp); \
	} while(0);
#endif
